...
def CalculerCout(cout):
    ...
    if cout.Type == "Direct":
        ...
        if cout.isFixed == True:
            cout.resultat = list(np.transpose(DictTableaux['Couts fixes'])[0])

def CalculerCoutLie(cout,source):
    cout.resultat = list(np.array(source.resultat)*cout.prcCalcul)